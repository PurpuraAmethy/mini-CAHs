...
REPO="${MINI-CAH_REPO:-https://github.com/martin-los/miniCAH.git}"
INSTALL_DIR="${MINI-CAH_INSTALL_DIR:-$HOME/.miniCAH-agent}"
BRANCH="${MINI-CAH_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[miniCAH]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[miniCAH]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[miniCAH]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[miniCAH] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
    printf "\n${BOLD}╔══════════════════════════════════════════╗${RESET}\n"
    printf   "${BOLD}║        miniCAH  一键安装                    ║${RESET}\n"
    printf   "${BOLD}╚══════════════════════════════════════════╝${RESET}\n\n"
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    BIN_DIR="${MINI-CAH_BIN_DIR:-$HOME/.local/bin}"
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/miniCAH"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/miniCAH" "\$@"
EOF
    chmod +x "$LAUNCHER"

    printf "\n"
    success "miniCAH 安装完成！"
    printf "\n"
