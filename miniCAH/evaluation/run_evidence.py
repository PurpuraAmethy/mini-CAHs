"""Read-only helpers for MiniCAH run/session evidence.

These helpers intentionally do not import or instantiate the MiniCAH runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.miniCAH/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".miniCAH" / "runs")
        ...
        session_path = _latest_file(workspace / ".miniCAH" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".miniCAH" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
