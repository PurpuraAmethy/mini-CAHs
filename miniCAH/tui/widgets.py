...
class WelcomeBanner(Static):
    ...
    def render(self) -> Text:
        ...
        rows = [
            Text.assemble(
                Text("miniCAH", style=f"bold {accent}"),
                Text("  local coding agent", style=muted),
            ),
            Text(""),
        ]
        rows.extend(Text(line, style=accent) for line in MINI-CAH_MARK)
        rows.extend(
            [
                Text(""),
                Text.assemble(
...
class InputBar(Static):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.input = Input(placeholder="Ask miniCAH or type /help")
    ...
    def set_busy(self, busy: bool) -> None:
        ...
        self.input.placeholder = (
            "miniCAH is working..." if busy else "Ask miniCAH or type /help"
        )
