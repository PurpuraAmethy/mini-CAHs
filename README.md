...
miniCAH 跑在本地仓库里，接上一个模型 provider，就能读代码、跑命令、改文件、
保留运行证据，并把有价值的上下文沉淀成本地记忆。

</div>

<p align="center">
  <img src="assets/screenshots/miniCAH-tui-intro.png" alt="miniCAH TUI 启动界面" width="960">
</p>
...
miniCAH 是一个本地终端里的 coding agent，运行在你的仓库上下文里。一次 agent 运行会被拆成几个可观察的部分：
...
- **session / run evidence**：对话、事件流、trace、report 都写到本地 `.miniCAH/`。
- **memory / dream**：把 daily log 整理成长期 topic，下次 session 可以继续用。

miniCAH 关注本地 coding agent 的工程边界：配置清楚、任务能续接、结果能复盘。
...
| --- | --- |
| ![miniCAH TUI 工具表](assets/screenshots/miniCAH-tui-tools.png) | ![miniCAH TUI skills 和 help](assets/screenshots/miniCAH-tui-skills-help.png) |

| Memory 和 durable topics | Slash command 工作区 |
| --- | --- |
| ![miniCAH TUI memory 和 skills](assets/screenshots/miniCAH-tui-memory-skills.png) | ![miniCAH TUI slash command 补全](assets/screenshots/miniCAH-tui-latest.png) |
...
```bash
curl -fsSL https://raw.githubusercontent.com/martin-los/miniCAH/main/install.sh | bash
```

源码安装：

```bash
git clone https://github.com/martin-los/miniCAH.git
cd miniCAH
pip install -e .
```
...
miniCAH 启动前先解析一个 **provider profile**。一个 profile 主要由四项组成：
...
```text
CLI 参数 > 环境变量 > 项目 .miniCAH.toml > 全局 ~/.config/miniCAH/config.toml > 代码默认值
```
...
```bash
cp .miniCAH.toml.example .miniCAH.toml
$EDITOR .miniCAH.toml
```

`.miniCAH.toml` 默认被 `.gitignore` 忽略，不要把真实 key 提交进 git。
...
```bash
export MINI-CAH_PROVIDER=deepseek
...
```bash
export MINI-CAH_API_KEY=sk-...
export MINI-CAH_BASE_URL=https://api.openai.com/v1
export MINI-CAH_MODEL=gpt-5.4
```
...
```bash
miniCAH --provider openai --model gpt-5.4 --base-url https://api.openai.com/v1
miniCAH --provider deepseek --approval ask --max-steps 80
miniCAH --config /path/to/custom.toml --cwd /path/to/repo
```
...
```bash
miniCAH                              # 默认 Textual TUI
miniCAH --repl                       # 普通终端 REPL
miniCAH "找出测试失败的根因"          # one-shot 任务
miniCAH --resume latest              # 续接最近 session
miniCAH --cwd /path/to/repo          # 指定工作目录
```

常用运行参数：

```bash
miniCAH --approval ask               # shell / 写文件前询问
miniCAH --approval auto              # 普通操作自动通过
miniCAH --approval never             # 非交互模式
miniCAH --sandbox best_effort        # 尽量隔离 shell 命令
...
| --- | --- |
| 项目配置 | `.miniCAH.toml` |
| 全局配置 | `~/.config/miniCAH/config.toml` |
| 会话历史 | `.miniCAH/sessions/<id>.json` |
| 事件流 | `.miniCAH/sessions/<id>.events.jsonl` |
| 运行证据 | `.miniCAH/runs/<run_id>/` |
| 记忆索引 | `.miniCAH/memory/MEMORY.md` |
| Daily logs | `.miniCAH/memory/logs/YYYY/MM/YYYY-MM-DD.md` |
| Durable topics | `.miniCAH/memory/topics/*.md` |
| 用户 skills | `~/.miniCAH/skills/<name>/SKILL.md` |
| 项目 skills | `skills/<name>/SKILL.md` 或 `.miniCAH/skills/<name>/SKILL.md` |
...
MINI-CAH_LIVE_SMOKE=1 pytest tests/test_release_smoke.py -q
```
...
| 入口 | 内容 |
| --- | --- |
| [配置](docs/configuration.md) | provider profile、`.miniCAH.toml`、环境变量和 sandbox 配置。 |
