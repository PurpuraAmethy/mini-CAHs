...
mini-CAHs 跑在本地仓库里，接上一个模型 provider，就能读代码、跑命令、改文件、
...
mini-CAHs 是一个本地终端里的 coding agent，运行在你的仓库上下文里。一次 agent 运行会被拆成几个可观察的部分：
...
mini-CAHs 关注本地 coding agent 的工程边界：配置清楚、任务能续接、结果能复盘。
...
mini-CAHs 启动前先解析一个 **provider profile**。一个 profile 主要由四项组成：
...
| 会话历史 | `.CAHs/sessions/<id>.json` |
| 事件流 | `.CAHs/sessions/<id>.events.jsonl` |
| 运行证据 | `.CAHs/runs/<run_id>/` |
| 记忆索引 | `.CAHs/memory/MEMORY.md` |
| Daily logs | `.CAHs/memory/logs/YYYY/MM/YYYY-MM-DD.md` |
| Durable topics | `.CAHs/memory/topics/*.md` |
| 用户 skills | `~/.CAHs/skills/<name>/SKILL.md` |
| 项目 skills | `skills/<name>/SKILL.md` 或 `.CAHs/skills/<name>/SKILL.md` |
