...
class MiniCAHTuiApp(App):
    """Textual shell for the existing MiniCAH runtime.

    The TUI is deliberately a presentation layer: CLI argument parsing and agent
    construction still live in `miniCAH.cli`, while turns are driven through the
    same `Engine.run_turn()` generator that powers the plain REPL.
    """

    CSS = MINI-CAH_TUI_CSS
    BINDINGS = [
